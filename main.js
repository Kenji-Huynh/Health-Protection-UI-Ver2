document.addEventListener("DOMContentLoaded", function () {
  ...
  function createFallingObjects() {
    ...
    // Số lượng vật thể rơi - tăng lên để nhiều lá hơn
    const objectCount = 40;

    // Các kích thước khác nhau - thêm kích thước lớn hơn để đa dạng
    const sizes = [10, 15, 20, 25, 30, 35];

    // Tốc độ khác nhau (thời gian animation) - GIẢM xuống để nhanh hơn
    const speeds = [8, 10, 12, 15, 18];

    // Độ trễ khác nhau - giảm thời gian giữa các lá
    const delays = Array.from({ length: objectCount }, (_, i) => i * 0.2);
    ...
    for (let i = 0; i < objectCount; i++) {
      ...
      // Thêm lớp leaf cho 70% các vật thể (tăng tỷ lệ lá)
      if (Math.random() > 0.3) {
      ...
      // Thêm vị trí bắt đầu theo chiều dọc ngẫu nhiên
      object.style.top = `${Math.random() * 50 - 50}%`;
      ...
      // Thêm hướng xoay ngẫu nhiên
      const rotation = Math.random() > 0.5 ? "normal" : "reverse";
      object.style.animationDirection = rotation;
